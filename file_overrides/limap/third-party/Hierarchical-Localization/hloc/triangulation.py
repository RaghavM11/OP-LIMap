...
def geometric_verification(image_ids: Dict[str, int],
    ...
    for name0 in tqdm(pairs):
        ...
        for name1 in pairs[name0]:
            ...
            qvec_01, tvec_01 = pycolmap.relative_pose(image0.qvec, image0.tvec, image1.qvec,
                                                      image1.tvec)
            _, errors0, errors1 = compute_epipolar_errors(qvec_01, tvec_01, kps0[matches[:, 0]],
                                                          kps1[matches[:, 1]])
            valid_matches = np.logical_and(errors0 <= max_error * noise0 / cam0.mean_focal_length(),
...
def run_triangulation(
    ...
    with OutputCapture(verbose):
        with pycolmap.ostream():
            reconstruction = pycolmap.triangulate_points(reference_model,
                                                         database_path,
                                                         image_dir,
                                                         model_path,
                                                         options=options)
...
def main(
    ...
    reconstruction = run_triangulation(sfm_dir, database, image_dir, reference, verbose,
                                       mapper_options)
...
def parse_option_args(args: List[str], default_options) -> Dict[str, Any]:
    ...
    for arg in args:
        ...
        if not hasattr(default_options, key):
            raise ValueError(f'Unknown option "{key}", allowed options and default values'
...
if __name__ == '__main__':
    ...
    mapper_options = parse_option_args(args.pop("mapper_options"),

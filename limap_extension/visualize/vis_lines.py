...
def open3d_get_cameras(imagecols,
                       color=[1.0, 0.0, 0.0],
                       ranges=None,
...
def rerun_vis_3d_lines(lines, width=0.01, ranges=None, scale=1.0):
    import rerun as rr
    rr.init("LIMAP Line Visualization", spawn=False)
    rr.serve()
    import time
    time.sleep(8)
    line_strips = rerun_get_line_strips(lines, ranges=ranges, scale=scale)
    rr.log("lines", rr.LineStrips3D(strips=line_strips, radii=width, colors=[1.0, 0.0, 0.0]))

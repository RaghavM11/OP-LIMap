...
class OpticalFlow:
    ...
    def load_model(self, path, args):
        ...
        weights = torch.load(path, map_location=DEVICE_DEFAULT)
        self.model = torch.nn.DataParallel(self.model)
        self.model.load_state_dict(weights)
        self.model.to(DEVICE_DEFAULT)

    def infer_flow(self, fr1, fr2, return_unpadded_flow: bool = True):
        self.model.eval()
        with torch.no_grad():
            fr1 = torch.from_numpy(fr1).permute(2, 0, 1).float()[None].to(DEVICE_DEFAULT)
            fr2 = torch.from_numpy(fr2).permute(2, 0, 1).float()[None].to(DEVICE_DEFAULT)
...
if __name__ == '__main__':
    ...
    for i in range(357):
        ...
        if (j > 356):
            break
        path1 = REPO_PATH / 'datasets/ocean/Hard/P006/image_left/'
        path2 = REPO_PATH / 'datasets/ocean/Hard/P006/depth_left/'
        path5 = REPO_PATH / 'datasets/ocean/Hard/P006/pose_left.txt'
        if i < 10:
            img_num = '00000' + str(i)
        elif i < 100:
            img_num = '0000' + str(i)
        ...
        img_num = '000091'
        img_num2 = '000092'
        fr1 = np.array(Image.open(path1 / f"{img_num}_left.png"))
        # print(fr1)
        fr2 = np.array(Image.open(path1 / f"{img_num2}_left.png"))
        print(type(fr1), type(fr2))
        depth1 = np.load(path2 / f"{img_num}_left_depth.npy")
        depth2 = np.load(path2 / f"{img_num2}_left_depth.npy")
        ...
        mask_rgb, mask_grey = motion_segmentation(fr1, depth1, fr2, depth2, cam_pose[i, :],
                                                  cam_pose[j, :])
        ...
        # plt.imshow(fr2)
        # plt.show()
        # cv2.imwrite(REPO_PATH / 'datasets/P007/masks_left/' + img_num + '_mask.png', mask_grey)
        break
